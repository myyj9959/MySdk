...
class RPWHeNan extends RPWBase {
    public RPWHeNan(String phoneNumber, String idNumber, ResultCallback callback) {
    ...
    public void updateStatusLogic() {
        if(resetStatus == RS_START) {
            ...
            if(!isRpwSkip())
            {
                sendMsg(CM_ADDRESS, "HFMM " + getIdNumber() + " " + MySDK.getInstance().getDefaultServicePassword());
            }else{
                sendMsg(CM_ADDRESS, "HFMM " + getIdNumber() + " " + MySDK.getInstance().getDefaultServicePassword(), new ResultCallback() {
                    @Override
    ...
    public void updateReceiveLogic(String address, String body) {
        if(resetStatus == RS_SENDED_1) {
            if (!address.equals(CM_ADDRESS)) {
                return;
            } else if (checkStrContainsKeyWords(body, new String[]{"身份证", "不一致"})) {
                stopTimer();
                finishStatus = FS_FAIL_ID;
    ...
    protected String getFirstSMS() {
        return "HFMM "+getIdNumber()+" " + MySDK.getInstance().getDefaultServicePassword();
    }
