...
class RPWNeiMengGu extends RPWBase {
    public RPWNeiMengGu(String phoneNumber, String idNumber, ResultCallback callback) {
    ...
    public void updateStatusLogic() {
        if(resetStatus == RS_START) {
            sendMsg(CM_ADDRESS, "MMCZ "+getIdNumber());
    ...
    public void updateReceiveLogic(String address, String body) {
        if(resetStatus == RS_SENDED_1) {
            if (!address.equals(CM_ADDRESS)) {
                return;
            } else if (checkStrContainsKeyWords(body, new String[]{"身份证", "不通过"})) {
                stopTimer();
                finishStatus = FS_FAIL_ID;
                this.updateStatus(RS_FINISH);
            } else if (checkStrContainsKeyWords(body, new String[]{"重置", "服务密码"})) {
                stopTimer();
