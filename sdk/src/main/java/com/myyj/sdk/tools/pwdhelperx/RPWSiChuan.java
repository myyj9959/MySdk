...
class RPWSiChuan extends RPWBase {
    public RPWSiChuan(String phoneNumber, String idNumber, ResultCallback callback) {
    ...
    public void updateReceiveLogic(String address, String body) {
        if(resetStatus == RS_SENDED_1) {
            if (!address.equals(CM_ADDRESS)) {
                return;
            } else if (checkStrContainsKeyWords(body, new String[]{"证件", "不正确"})) {
                stopTimer();
                finishStatus = FS_FAIL_ID;
                this.updateStatus(RS_FINISH);
            } else if (checkStrContainsKeyWords(body, new String[]{"变更", "成功重置"})) {
                stopTimer();
