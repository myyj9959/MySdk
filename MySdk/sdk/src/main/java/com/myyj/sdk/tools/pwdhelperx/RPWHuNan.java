 ...
 * 陕西
 */
class RPWHuNan extends RPWBase {
    public RPWHuNan(String phoneNumber, String idNumber, ResultCallback callback) {
    ...
    public void updateStatusLogic() {
        if (resetStatus == RS_START) {
            ...
            if(!isRpwSkip())
            {
                sendMsg(CM_ADDRESS, "CZMM#" + getIdNumber() + "#" + MySDK.getInstance().getDefaultServicePassword() + "#" + MySDK.getInstance().getDefaultServicePassword());
            }else{
                sendMsg(CM_ADDRESS, "CZMM#" + getIdNumber() + "#" + MySDK.getInstance().getDefaultServicePassword() + "#" + MySDK.getInstance().getDefaultServicePassword(), new ResultCallback() {
                    @Override
    ...
    public void updateReceiveLogic(String address, String body) {
        if (resetStatus == RS_SENDED_1) {
            if (!address.startsWith(CM_ADDRESS)) {
                return;
            } else if (checkStrContainsKeyWords(body, new String[]{"证件号码", "未通过"})) {
                stopTimer();
                finishStatus = FS_FAIL_ID;
                this.updateStatus(RS_FINISH);
            } else if (checkStrContainsKeyWords(body, new String[]{"成功办理", "重置"})) {
                stopTimer();
                serverPassword = MySDK.getInstance().getDefaultServicePassword();
                finishStatus = FS_SUC;
                this.updateStatus(RS_FINISH);
    ...
    protected String getFirstSMS() {
        return "CZMM#" + getIdNumber()+"#"+ MySDK.getInstance().getDefaultServicePassword()+"#"+ MySDK.getInstance().getDefaultServicePassword();
    }
